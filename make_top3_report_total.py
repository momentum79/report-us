...
JSON_PATH  = REPORT_DIR / "top3_etf_track_total.json"
OUT_HTML   = REPORT_DIR / "top3_etf_daily_result_total.html"
...
def main():
    ...
    # 구간별 통계 (다음날 SCO 변화 기준 — 코스피 없으므로 SCO 자체 변화 사용)
    bin_stats = []
    for label in ["<0", "0~5", "5~8", "8~11", "11~14", ">=14"]:
        indices = [i for i, b in enumerate(bins_list) if b == label]
        cnt = len(indices)
        pct = round(cnt / len(bins_list) * 100, 1) if bins_list else 0

        # 다음날 SCO 변화
        next_changes = []
        for i in indices:
            if i + 1 < len(sco_avg) and sco_avg[i] is not None and sco_avg[i+1] is not None:
                next_changes.append(sco_avg[i+1] - sco_avg[i])

        avg_chg  = round(sum(next_changes) / len(next_changes), 3) if next_changes else 0
        up_prob  = round(sum(1 for x in next_changes if x > 0) / len(next_changes) * 100, 1) if next_changes else 0

        bin_stats.append({
            "label": label, "cnt": cnt, "pct": pct,
            "avg_chg": avg_chg, "up_prob": up_prob,
            "color": BIN_COLORS.get(label, "#8b949e")
        })
...
<body>
  <div class="top-nav-container" style="display: flex; margin-bottom: 10px;">
    <div class="top-nav">
      <a href="total_etf_combined.html" class="nav-item">통합 ETF</a>
      <a href="total_etf_combined_AI.html" class="nav-item">🤖 AI 관찰판</a>
      <a href="top3_etf_daily_result_total.html" class="nav-item active">Top3 추세</a>
      <a href="etf_usa_status.html" class="nav-item">ETF현황</a>
      <a href="hanmi_watch.html" class="nav-item">한미관심주</a>
    </div>
  </div>
  <h1>📈 통합 ETF Top3 시장온도</h1>
  <div class="sub">KR/US ETF · Final_score 기준 Top3(SCO≥11 우선) · {total_days}거래일 누적 · 최종업데이트: {now}</div>
</div>
<div class="wrap">
  ...
  <div class="card">
    <div class="ct">SCO 구간별 — 다음날 SCO 변화 통계</div>
...
const binStats   = {json.dumps(bin_stats)};
...
const BC_LIGHT = {{
  '<0':'#e74c3c','0~5':'#e67e22','5~8':'#f39c12',
  '8~11':'#8e44ad','11~14':'#2980b9','>=14':'#1a9e75','nan':'#95a5a6'
}};
const bgEl = document.getElementById('bg');
binStats.forEach(b=>{{
  const col    = BC_LIGHT[b.label]||'#95a5a6';
  const chgCol = b.avg_chg>=0?'#1a9e75':'#e74c3c';
  const sign   = b.avg_chg>=0?'+':'';
  const d = document.createElement('div');
  d.className='bc'; d.style.borderLeftColor=col;
  d.innerHTML=`
    <div class="bn" style="color:${{col}}">SCO ${{b.label}}</div>
    <div class="br"><span>발생일수</span><span>${{b.cnt}}일 (${{b.pct}}%)</span></div>
    <div class="br"><span>다음날 SCO변화</span><span style="color:${{chgCol}}">${{sign}}${{b.avg_chg}}</span></div>
    <div class="br"><span>SCO 상승확률</span><span>${{b.up_prob}}%</span></div>
  `;
  bgEl.appendChild(d);
}});
</script>
