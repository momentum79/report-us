...
방식:
  ...
  - 기존 CSV 에 없는 날짜는 추가
  - 기존 날짜라도 AG~AJ(sco_strong/sco_mid/sco_weak/sco_neg) 중 하나가 비어 있으면 재계산 후 덮어씀
...
def main():
    ...
    if os.path.exists(OUT_TRACK_CSV):
        ...
        need_cols = ['sco_strong', 'sco_mid', 'sco_weak', 'sco_neg']
        for col in need_cols:
            if col not in existing_df.columns:
                existing_df[col] = np.nan

        existing_dates = set(existing_df['date'].tolist())
        missing_agaj_dates = set(
            existing_df.loc[
                existing_df[need_cols].isna().any(axis=1),
                'date'
            ].tolist()
        )

        print(f"기존 CSV: {len(existing_df)}행, {len(existing_dates)}개 날짜")
        print(f"AG~AJ 빈 날짜: {len(missing_agaj_dates)}개")
    else:
    ...
    new_days = [
        ...
        if (d not in existing_dates) or (d in missing_agaj_dates)
    ]
    print(f"전체 거래일: {len(trading_days)}일 / 처리 대상: {len(new_days)}일")

    if not new_days:
        print("추가/보정할 날짜 없음. 종료.")
        return
