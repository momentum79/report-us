...
OUT_DIR      = os.path.join(BASE_DIR, "report-us")
...
# 봇 표시명 + 보유단위 ("day"=스윙 일, "min"=당일 분)
BOT_ORDER = ["통합", "주도주", "한TR", "한TR저", "ABC-VCP", "수동매매", "통합ETF",
             "2X단타", "5min저고단타", "미국VCP", "미TR", "미TR저", "미국수동",
             "ROCKET", "저점사다리", "기타(8042)"]
BOT_UNIT  = {
    "주도주": "day", "ROCKET": "day", "한TR": "day", "한TR저": "day",
    "ABC-VCP": "day", "수동매매": "day", "통합ETF": "day",
    "미국VCP": "day", "미TR": "day", "미TR저": "day", "미국수동": "day",
    "2X단타": "min", "5min저고단타": "min", "기타(8042)": "min", "저점사다리": "min",
    "통합": "mix",
}
...
def resolve_owner(acct, strategy):
    """(acct, strategy) → 소유 봇 표시명. 매수 fill 기준 소유 확정용."""
    ...
    if acct == "1887":
        ...
        if s == "leader":
            return "주도주"
        if s == "kr_pine_buy":
            return "한TR"                       # TR 추세셋 (0order/tr/kr_pine_buy_1887.py)
        if s == "kr_pine_lowbuy":
            return "한TR저"                     # TR 저점셋 (0order/tr/kr_pine_lowbuy_1887.py)
        return "수동매매"                       # manual/미태깅 → 사람이 직접 낸 주문
    if acct == "1887US":
        if s == "usvcp":
            return "미국VCP"
        if s == "us_pine_buy":
            return "미TR"                       # TR 추세셋 (0order/tr/us_pine_buy_1887.py)
        if s == "us_pine_lowbuy":
            return "미TR저"                     # TR 저점셋 (0order/tr/us_pine_lowbuy_1887.py)
        return "미국수동"
    if acct == "8042ETF":
...
def build_round_trips(fills):
    """계좌(silo)+종목별 평균단가 lifecycle 재구성.
    flat→매수…→flat 한 사이클 = 1 라운드트립(완결거래).
    매수가 소유봇 확정, 매도는 흡수(태그 무시) → rocket 등 정확 귀속."""
    open_pos = {}      # (acct, code) → dict
    trips = []
    ...
    for f in fills:
        ...
        if f["side"] == "buy":
            ...
            if p is None or p["held"] <= 0:
                # 새 포지션 시작 — 소유봇 확정
                p = new_pos(resolve_owner(f["acct"], f["strategy"]), f["name"])
                open_pos[key] = p
                p["first_buy_dt"] = f["dt"]
            ...
            if p["held"] <= 0:
                ...
                if p["buy_qty"] > 0 and p["sell_qty"] > 0:
                    ...
                    if avg_entry > 0:
                        ...
                        trips.append({
                            "bot": p["owner"], "code": key[1], "name": p["name"],
...
def build_report(trips, weeks_limit=None):
    ...
    if weeks_limit:
        keep = set(all_weeks[-weeks_limit:])
    ...
    for bot in BOT_ORDER:
        bt = by_bot.get(bot, [])
        # 통합·ROCKET 은 거래 0 이라도 항상 슬롯 유지(레이아웃 자리 예약). 그외는 생략.
        if bot not in ("통합", "ROCKET") and not bt:
            continue
...
def write_csv(report):
    with open(OUT_CSV, "w", newline="", encoding="utf-8-sig") as f:
        ...
        for b in report["bots"]:
            ...
            w.writerow([])
    print(f"  ✅ {os.path.relpath(OUT_CSV, BASE_DIR)}")
...
def write_xlsx(report):
    ...
    wb.save(OUT_XLSX)
    print(f"  ✅ {os.path.relpath(OUT_XLSX, BASE_DIR)}")


def main():
    print("── 주간성과 집계 (intraday_signals fill 스트림 기반) ──")
    fills = load_fills()
    print(f"  fill 이벤트: {len(fills)}건")
    trips = build_round_trips(fills)
    print(f"  완결 라운드트립: {len(trips)}건")
