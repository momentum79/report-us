...
def build_target_weights(held_list: list, data: list, s_data: dict,
                         ...
                         stats_status: str = STATS_OK) -> bool:
    ...
    for tk in held_list:
        ...
        if pct < 0:
            ...
            return False
        targets.append({
            "market": classify_market(tk),   # 'KR' | 'US'
            "ticker": tk,
...
def main():
    ...
    final_order_html  = build_final_order_table(held_list, data, s_data,
                                                held_status, stats_status)
    # 주문기(allone)용 '전략 비중' JSON 생성(절대수량 아님). 각 계좌가 자체 총자산으로 수량 산출.
    build_target_weights(held_list, data, s_data, held_status, stats_status)
