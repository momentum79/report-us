...
HOLDING_FILE = "D:\\py\\buy_us_interest_holding.txt"
OUTPUT_FILE  = "D:\\py\\buy_us_interest.txt"
...
# ====== US Interest 리스트: 0total_tv_input.txt 에서 동적 로딩 ======
def load_us_etfs_from_txt(filepath: str) -> list:
    """
    0total_tv_input.txt 에서 두 구간의 US 티커를 읽어온다.

    1) -----미Main  ~  ------ US Sector 우량주 Top30  사이의 티커 (KRX: 제외, 전부)
    2) ------ US Sector 우량주 Top30  바로 아래 티커 10개
    ...
    idx_main   = find_section("미Main")
    idx_top30  = find_section("US Sector 우량주 Top30")

    if idx_main == -1 or idx_top30 == -1:
        print(f"⚠️  섹션을 찾지 못했습니다. (미Main={idx_main}, Top30={idx_top30})")
        return []

    tickers = []

    # ── 구간 1: 미Main 헤더 다음 줄 ~ Top30 헤더 바로 위 ──
    for line in lines[idx_main + 1 : idx_top30]:
        if not line or line.startswith("-") or line.startswith("KRX:"):
            continue
        tickers.append(line)

    # ── 구간 2: Top30 헤더 다음 줄부터 10개 ──
    count = 0
    for line in lines[idx_top30 + 1 :]:
        if count >= 10:
            break
        if not line or line.startswith("-") or line.startswith("KRX:"):
            continue
    ...
    print(f"✅  티커 로딩 완료 ({filepath}): {len(unique)}개 → {unique}")
    return unique


TV_INPUT_FILE = r"D:\py\0txt\0total_tv_input.txt"
US_ETFS = load_us_etfs_from_txt(TV_INPUT_FILE)
...
def main():
    ...
    res["Rank_rtn"] = res["수익률(%)"].fillna(res["수익률(%)"].min()).rank(pct=True)
    res["Final_score"] = res["Norm_sco"] * 0.85 + res["Rank_rtn"] * 0.15
    ...
    holding_df       = res[res['Ticker'].isin(holding_symbols)].copy()
    holding_df       = holding_df.sort_values(by='Final_score', ascending=False)
    final_holding_df = holding_df.head(6).copy()
    ...
    try:
        EXTRA_OUT = r"D:\py\0txt\buy_us_interest.txt"
    ...
    stats_path = os.path.join("D:\\py\\report-us", "us_signal_stats_interest.json")
    try:
    ...
    low_signal_path = os.path.join("D:\\py\\report-us", "us_interest_low_signals.json")
    try:
...
if __name__ == "__main__":
    report_file_path = "D:\\py\\report-us\\report_us_interest.txt"
    try:
