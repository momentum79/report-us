...
def build_final_order_table(held_list: list, data: list, s_data: dict) -> str:
    ...
    rows_html = []
    rebalancing_rows = []
    order_build_failed = False   # 9-11: 하나라도 실패하면 기존 파일 덮어쓰지 않음
    failed_tickers = []
    if ASSET_8042 <= 0:
    ...
    for tk in held_list:
        ...
        need_qty = (ASSET_8042 > 0 and pct_for_qty > 0)
        if need_qty and price and price > 0:
            if use_krx_price:
                ...
                qty = int(ASSET_8042 * pct_for_qty / 100 / USD_KRW_EFFECTIVE / price)
                pension_qty = int(PENSION_ASSET * pct_for_qty / 100 / USD_KRW_EFFECTIVE / price)
                qty_krw = qty * price * USD_KRW_EFFECTIVE
                pension_krw = pension_qty * price * USD_KRW_EFFECTIVE
    ...
    # 9-11: all-or-nothing. 하나라도 실패했거나 산출된 행이 없으면 기존 파일 보존.
    if order_build_failed or not rebalancing_rows:
        print(f"[리밸런싱] 생성 실패 → 기존 파일 유지(덮어쓰기 안 함). "
