...
REPORT_TXT = pathlib.Path(__file__).resolve().parent / "report_kor_etf_vol.txt"
OUT_HTML   = pathlib.Path(__file__).resolve().parent / "kor_etf_vol.html"
GANN_FIRE_JSON = pathlib.Path(__file__).resolve().parent / "kr_etf_gann_fire_set_vol.json"
LOW_HISTORY_FILE = pathlib.Path(__file__).resolve().parent / "kor_etf_low_history_vol.json"
REBALANCING_TXT = pathlib.Path(r"D:\py\0order\00_etf_korea_rebalancing_vol.txt")
...
def get_monthly_top3() -> tuple[str, list[str]]:
    ...
    try:
        today = datetime.date.today()
        target_key = f"{today.year}.{today.month:02d}"  # '2026.04'
...
"""
    ...
    stats_html = ""
    stats_file = pathlib.Path(__file__).resolve().parent / "kr_signal_stats_vol.json"

    low_signals_dict = {}
    idx_rel_map = {}
    lime_thresh_map = {}
    low_signal_file = pathlib.Path(__file__).resolve().parent / "kr_etf_low_signals_vol.json"
...
<p style="margin:0; font-size:0.95em;">
    ...
    if stats_file.exists():
        try:
            ...
            sc3_ok = True
            sc3_status_path = pathlib.Path(__file__).resolve().parent / "kr_sc3_status_vol.json"
...
<div class="container-all">
    <div class="top-nav-container">
        <div class="top-nav">
            <a href="kor_etf.html" class="nav-item">한국 ETF (원본)</a>
            <a href="kor_etf_vol.html" class="nav-item active">📉 한국 ETF (변동성)</a>
    ...
    <div class="top-layout">
        <div class="left-content">
            <h1>🇰🇷 Korea ETF Report <span style="font-size:0.6em;color:#8e44ad;">📉 변동성 조정판</span></h1>
...
"""
    ...
    print(f"[OK] kor_etf_vol.html updated at {OUT_HTML}")
